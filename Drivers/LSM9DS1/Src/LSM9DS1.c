...
void LSM9DS1_Init(void)
{
	...
	asm("NOP");
	HAL_Delay(50);
	status = HAL_I2C_Mem_Read(&LSM9DS1_I2C_PORT, LSM9DS1_AG_ADDR, CTRL_REG1_G,  1, &RegVal1, 1 , 10);
	asm("NOP");
	status = HAL_I2C_Mem_Read(&LSM9DS1_I2C_PORT, LSM9DS1_AG_ADDR, CTRL_REG6_XL, 1, &RegVal2, 1 , 10);
	asm("NOP");
...
HAL_StatusTypeDef LSM9DS1_ReadAcc(float *xAcc, float *yAcc, float *zAcc)
{
	...
	status = HAL_I2C_Mem_Read(&LSM9DS1_I2C_PORT, LSM9DS1_AG_ADDR, WHO_AM_I_XG, 1, (uint8_t*)retVal, 1 , 10);
	asm("NOP");

	status = HAL_I2C_Mem_Read(&LSM9DS1_I2C_PORT, LSM9DS1_AG_ADDR, OUT_X_L_XL, 1, (uint8_t*)retVal, 2 , 10);
...
HAL_StatusTypeDef LSM9DS1_ReadGyro(float *xRot, float *yRot, float *zRot)
{
	...
	status = HAL_I2C_Mem_Read(&LSM9DS1_I2C_PORT, LSM9DS1_AG_ADDR, OUT_X_L_G, 1, &data_rec, 2 , 10);
